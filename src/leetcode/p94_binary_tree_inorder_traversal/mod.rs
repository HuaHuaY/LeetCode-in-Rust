    ...
    pub fn inorder_traversal(root: Option<Rc<RefCell<TreeNode>>>) -> Vec<i32> {
        fn inorder(root: Option<Rc<RefCell<TreeNode>>>, result: &mut Vec<i32>) {
            if let Some(root) = root {
                ...
                inorder(root.left.take(), result);
                result.push(root.val);
                inorder(root.right.take(), result);
            }
        }
        let mut result = vec![];
...
mod tests {
    ...
    fn test1() {
        ...
        assert_eq!(Solution::inorder_traversal(root), [1, 3, 2]);
    }

    #[test]
    fn test2() {
        let root = None;
        assert_eq!(Solution::inorder_traversal(root), []);
    }

    #[test]
    fn test3() {
        let root = Some(Rc::new(RefCell::new(TreeNode::new(1))));
        assert_eq!(Solution::inorder_traversal(root), [1]);
    }
}
